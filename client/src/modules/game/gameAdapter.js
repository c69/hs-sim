 ...
 * Used to make action and card(entity) matching easier/faster (currently happens in
 * groupByPlayerZones)
 * {
 *   [card_id]: action
 * }
...
function buildActionsByCardId(actions = []) {
  return actions.reduce((result, action, actionIndex) => {
    if (action.card_id) {
      Object.assign(result, { [action.card_id]: { ...action, actionIndex } });
    }
...
/**
 * 1. Creates an object with keys as ZONES and values are an array of entities to be shown on the
 * board
 * {
 *  HAND: [{}, {}, ...]
 *  PLAY: [{}, {}, ...]
...
export function groupByPlayerZones(entities, owner, actionsByCardId) {
  ...
  return playerEntities.reduce((result, entity) => {
    ...
    const zone = getCardZone(card, gameConstants.types.HERO);

    // set card action if it matches
    if (actionsByCardId[card.card_id]) {
      card.action = actionsByCardId[card.card_id];
    }

    if (result[zone]) {
      result[zone].push(card);
    } else {
      Object.assign(result, { [zone]: [card] });
    }
...
/**
 * Main game massager :)
 * Create an object with game ZONES for player and the opponent (Format convenient for Board
...
function massageGame(payload) {
  ...
  // TODO: this can be improved further. Right now groupByPlayerZones iterates over all possible
  // entities twice.
  return {
    ...payload,
    activePlayer: {
